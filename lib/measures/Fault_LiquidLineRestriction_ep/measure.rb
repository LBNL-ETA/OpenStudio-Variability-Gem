...
class LiquidLineRestriction < OpenStudio::Ruleset::WorkspaceUserScript
  ...
  def _check_fault_lvl(runner, coil_choice, fault_lvl)
    ...
    if fault_lvl < 0.0 || fault_lvl > 1.0
      ...
      runner.registerAsNotApplicable("Fault_LiquidLineRestriction_ep is not running for #{coil_choice}. Skipping......")
      return true
    end
    return 'continue'
  end
  ...
  def _write_ems_string(workspace, runner, user_arguments, coil_choice, fault_lvl, coilcooling, coiltype, time_constant, time_step, start_month, start_date, start_time, end_month, end_date, end_time)
    ...
    if coiltype == 1 #SINGLESPEED
      unless pass_string(coilcooling, 20).eql?('AirCooled')
        runner.registerError("#{coil_choice} is not air cooled. Impossible to continue in Fault_LiquidLineRestriction_ep. Exiting......")
        return false
      end
    end
    ##################################################
  ...
  def _return_err_message_for_not_unit(runner, existing_coils, coil_choice)
    runner.registerError("Measure Fault_LiquidLineRestriction_ep cannot find #{coil_choice}. Exiting......")
