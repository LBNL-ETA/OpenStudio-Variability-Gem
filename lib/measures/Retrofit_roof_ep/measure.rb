...
# start the measure
class RoofRetrofit < OpenStudio::Measure::EnergyPlusMeasure
  # human readable name
  def name
    # Measure name should be the title case of the class name.
    return 'Roof Retrofit'
  end

  # human readable description
  def description
    return 'This measure increase the thermal resistance of the roof by adding a movable insulation at user specified date.'
  end
  ...
  def arguments(workspace)
    ...
    arg_material_roughness.setDisplayName('Choose new roof insulation material roughness')
    ...
    arg_retrofit_month.setDefaultValue('8')
    ...
    # 4. List all exterior roofs for user to decide whether or not to add movable insulation
  ...
  def run(workspace, runner, user_arguments)
    ...
    retrofit_sch_type_limit_name = 'Roof Retrofit Sch Type - Fraction'
    retrofit_sch_name = 'Roof Retrofit Sch'
    retrofit_material_name = 'Roof Retrofit Material'
    retrofit_ineffective_day_sch_name = 'Roof Retrofit Ineffective Day Schedule'
    retrofit_effective_day_sch_name = 'Roof Retrofit Effective Day Schedule'
    retrofit_ineffective_week_daily_sch_name = "Roof Retrofit Schedule Week Rule - 1/1-#{retrofit_month}/#{retrofit_day}"
    retrofit_effective_week_daily_sch_name = "Roof Retrofit Schedule Week Rule - #{retrofit_month}/#{retrofit_day}-12/31"
    ...
    # array to hold new IDF objects needed for exterior roof retrofit
    string_objects = []
    ...
    runner.registerInfo("Retrofit -- Create SurfaceControl:MovableInsulation objects for selected roofs.")
    v_ws_building_surface = workspace.getObjectsByType('BuildingSurface:Detailed'.to_IddObjectType)
    v_ws_building_surface.each do |ws_surface|
      surface_name = ws_surface.getField(0).get
      surface_type = ws_surface.getField(1).get
    ...
    runner.registerInfo('Start to create movable insulation for roofs.')
...
RoofRetrofit.new.registerWithApplication
